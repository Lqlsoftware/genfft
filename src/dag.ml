...
(* true if node uses v *)
let node_uses v node = List.exists (Variable.same v) node.input_variables
...
let depends_on nodea nodeb =
  node_uses nodea.assigned nodeb || node_clobbers nodea nodeb.assigned
