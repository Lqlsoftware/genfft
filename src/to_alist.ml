...
let fetchAl = fetchState >>= fun (al, _, _) -> returnM al

let storeAl al =
  fetchState >>= fun (_, visited, visited') -> storeState (al, visited, visited')

let fetchVisited = fetchState >>= fun (_, v, _) -> returnM v

let storeVisited visited =
  fetchState >>= fun (al, _, visited') -> storeState (al, visited, visited')

let fetchVisited' = fetchState >>= fun (_, _, v') -> returnM v'

let storeVisited' visited' =
  fetchState >>= fun (al, visited, _) -> storeState (al, visited, visited')

let lookupVisitedM' key =
  fetchVisited' >>= fun table -> returnM (node_lookup key table)

let insertVisitedM' key value =
  fetchVisited' >>= fun table -> storeVisited' (node_insert key value table)
...
let counting f x =
  ...
  | Some count -> (
      let incr_cnt =
        fetchVisited >>= fun v' -> storeVisited (node_insert x (count + 1) v')
      in
      match x with
      (* Uminus is always inlined.  Visit child *)
      | Uminus y -> f y >> incr_cnt
      | _ -> incr_cnt )
  | None -> f x >> fetchVisited >>= fun v' -> storeVisited (node_insert x 1 v')
...
let with_temp_maybeM node x =
  ...
  | Some count ->
      if count = 1 && !Magic.inline_single then inlineM x else with_tempM x
  | None -> failwith "with_temp_maybeM"

type fma =
  | NO_FMA
  | FMA of expr * expr * expr (* FMA (a, b, c) => a + b * c *)
  | FMS of expr * expr * expr (* FMS (a, b, c) => -a + b * c *)
  | FNMS of expr * expr * expr
  ...
  memoizing lookupVisitedM' insertVisitedM'
    (function
      | x -> (
          match x with
          | Load v ->
              if Variable.is_temporary v then inlineM (Load v)
              else if Variable.is_locative v && !Magic.inline_loads then
                inlineM (Load v)
              else if Variable.is_constant v && !Magic.inline_loads_constants
              then inlineM (Load v)
              else with_tempM (Load v)
          | Num a ->
              if !Magic.inline_constants then inlineM (Num a)
              else with_temp_maybeM x (Num a)
          | NaN a -> inlineM (NaN a)
          | Store (v, x) ->
              expr_of_nodeM x
          ...
          | Plus a -> (
              ...
              | FMS (a, b, c) ->
                  ...
                  expr_of_nodeM b >>= fun b' ->
                  expr_of_nodeM c >>= fun c' ->
                  with_temp_maybeM x (Plus [ Times (b', c'); Uminus a' ])
              | FNMS (a, b, c) ->
                  expr_of_nodeM a >>= fun a' ->
                  expr_of_nodeM b >>= fun b' ->
                  expr_of_nodeM c >>= fun c' ->
                  with_temp_maybeM x (Plus [ a'; Uminus (Times (b', c')) ])
              | NO_FMA ->
                  mapM expr_of_nodeM a >>= fun a' ->
                  with_temp_maybeM x (Plus a') )
          | CTimes ((Load _ as a), b) when !Magic.generate_bytw ->
              expr_of_nodeM b >>= fun b' -> with_tempM (CTimes (a, b'))
          | CTimes (a, b) ->
              expr_of_nodeM a >>= fun a' ->
              expr_of_nodeM b >>= fun b' -> with_tempM (CTimes (a', b'))
          | CTimesJ ((Load _ as a), b) when !Magic.generate_bytw ->
              expr_of_nodeM b >>= fun b' -> with_tempM (CTimesJ (a, b'))
          | CTimesJ (a, b) ->
              expr_of_nodeM a >>= fun a' ->
              expr_of_nodeM b >>= fun b' -> with_tempM (CTimesJ (a', b'))
          | Times (a, b) -> (
          ...
          | Uminus a -> expr_of_nodeM a >>= fun a' -> inlineM (Uminus a') ))
    x

let expr_of_rootsM = mapM expr_of_nodeM

let peek_alistM roots = visit_rootsM roots >> expr_of_rootsM roots >> fetchAl
...
let dump print alist =
  ...
  List.iter
    (fun (Expr.Assign (v, x)) ->
      List.iter
        (fun y -> if Variable.is_locative y then print ("\t" ^ vs y ^ ";\n"))
        (Expr.find_vars x))
    alist;
  print "}\n";
