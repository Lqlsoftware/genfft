...
type variable =
  ...
  | Locative of (Unique.unique * Unique.unique * (int -> string) * int * string)
...
let is_temporary = function Temporary _ -> true | _ -> false

let is_locative = function Locative _ -> true | _ -> false
...
let same_class a b =
  ...
  | Constant (class_a, _), Constant (class_b, _) -> Unique.same class_a class_b
  | _ -> false
...
let make_constant class_token name = Constant (class_token, name)
...
let rec base62_of_int k =
  let x = k mod 62 and y = k / 62 in
  let c =
    if x < 10 then Char.chr (x + Char.code '0')
    else if x < 36 then Char.chr (x + Char.code 'a' - 10)
    else Char.chr (x + Char.code 'A' - 36)
  in
...
let varname_of_int k =
  if !Magic.compact then base62_of_int k else string_of_int k
