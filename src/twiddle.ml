...
let rec unroll_twfull l =
  match l with
  | [] -> []
  | (TW_FULL, v, n) :: b ->
      forall [] cons 1 n (fun i -> (TW_CEXP, v, i)) @ unroll_twfull b
  | a :: b -> a :: unroll_twfull b

let twinstr_to_c_string l =
  let one (op, a, b) = Printf.sprintf "{ %s, %d, %d }" (optostring op) a b in
...
let twinstr_to_simd_string vl l =
  ...
  in
  "{" ^ loop true (unroll_twfull l) ^ ",}"

let rec pow m n = if n = 0 then 1 else m * pow m (n - 1)

let rec is_pow m n = n = 1 || (n mod m = 0 && is_pow m (n / m))

let rec log m n = if n = 1 then 0 else 1 + log m (n / m)

let rec largest_power_smaller_than m i =
  if is_pow m i then i else largest_power_smaller_than m (i - 1)

let rec smallest_power_larger_than m i =
  if is_pow m i then i else smallest_power_larger_than m (i + 1)
...
let ctimes use_complex_arith a b =
  if use_complex_arith then Complex.ctimes a b else Complex.times a b

let ctimesj use_complex_arith a b =
  if use_complex_arith then Complex.ctimesj a b
  else Complex.times (Complex.conj a) b

let make_bytwiddle sign use_complex_arith g f i =
  if i = 0 then f i
  else if sign = 1 then ctimes use_complex_arith (g i) (f i)
  else ctimesj use_complex_arith (g i) (f i)
...
let twiddle_policy_load_all v use_complex_arith =
  ...
  and twidlen n = 2 * (n - 1)
  and twdesc r = [ (TW_FULL, v, r); (TW_NEXT, 1, 0) ] in
  (bytwiddle, twidlen, twdesc)
...
let twiddle_policy_log2 v use_complex_arith =
  let bytwiddle n sign w f =
    ...
    in
    make_bytwiddle sign use_complex_arith g f
  and twidlen n = 2 * log 2 (largest_power_smaller_than 2 ((2 * n) - 1))
  and twdesc n =
    List.flatten
      (List.map
         (fun i -> if i > 0 && is_pow 2 i then [ (TW_CEXP, v, i) ] else [])
         (iota n))
    @ [ (TW_NEXT, 1, 0) ]
  in
  (bytwiddle, twidlen, twdesc)
...
let twiddle_policy_log3 v use_complex_arith =
  let rec terms_needed i pi s n =
    if s >= n - 1 then i else terms_needed (i + 1) (3 * pi) (s + pi) n
  in
  let rec bytwiddle n sign w f =
    let nterms = terms_needed 0 1 0 n in
    let maxterm = pow 3 (nterms - 1) in
    let g =
      rec_array (3 * n) (fun self i ->
          if i = 0 then Complex.one
          else if is_pow 3 i then w (log 3 i)
          else if i = n - 1 && maxterm >= n then w (nterms - 1)
          else
            let x = smallest_power_larger_than 3 i in
            if i + i >= x then
              let x = min x (n - 1) in
              ctimesj use_complex_arith (self (x - i)) (self x)
            else
              let x = largest_power_smaller_than 3 i in
...
let twiddle_policy use_complex_arith = !current_twiddle_policy use_complex_arith
