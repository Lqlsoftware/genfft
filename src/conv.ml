...
let rec divmod n i =
  if i < 0 then
    let a, b = divmod n (i + n) in
    (a - 1, b)
  else (i / n, i mod n)

let unpolyphase m a i =
  let x, y = divmod m i in
  a y x
...
let rec conv na a nb b =
  let rec naive na a nb b i = sigma 0 na (fun j -> a j @* b (i - j))
  and recur na a nb b =
    if na <= 1 || nb <= 1 then naive na a nb b
    else
  ...
  and karatsuba1 na a nb b =
    ...
    and bo = p b 1
    and nbo = nb / 2 in
    let ae = infinite nae ae
    and ao = infinite nao ao
    and be = infinite nbe be
    and bo = infinite nbo bo in
    let aeo = lift2 ( @+ ) ae ao
    and naeo = nae
    and beo = lift2 ( @+ ) be bo
    and nbeo = nbe in
  ...
  and karatsuba2 na a nb b =
    ...
    let q = function
      | 0 -> lift2 ( @- ) c1 c2
      | 1 -> lift2 ( @- ) c1 c3
      | _ -> failwith "karatsuba2"
    in
    unpolyphase 2 q
  and karatsuba na a nb b =
    let m = na + nb - 1 in
    if m < !Magic.karatsuba_min then recur na a nb b
    else
  ...
  and via_circular na a nb b =
    let m = na + nb - 1 in
    if m < !Magic.circular_min then karatsuba na a nb b
    else
