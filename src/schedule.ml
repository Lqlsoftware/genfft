...
let makedag l =
  Dag.makedag (List.map (function Expr.Assign (v, x) -> (v, x)) l)
...
let cc dag inputs =
  Dag.for_all dag (fun node -> node.label <- infinity);

  (match inputs with a :: _ -> bfs dag a 0 | _ -> failwith "connected");

  return
    ( List.map to_assignment
        (List.filter (fun n -> n.label < infinity) (Dag.to_list dag)),
      List.map to_assignment
        (List.filter (fun n -> n.label == infinity) (Dag.to_list dag)) )
...
let rec connected_components alist =
  ...
  let inputs =
    List.filter (fun node -> Util.null node.predecessors) (Dag.to_list dag)
  in
...
let partition alist =
  ...
  let inputs = List.filter (fun node -> Util.null node.predecessors) dag'
  and outputs = List.filter (fun node -> Util.null node.successors) dag'
  and special_inputs = List.filter single_load dag' in
  let c =
    match !Magic.schedule_type with
    | 1 -> RED
    (* all nodes in the input partition *)
    | -1 -> BLUE
    (* all nodes in the output partition *)
    | _ -> BLACK
    (* node color determined by bisection algorithm *)
  in
  Dag.for_all dag (fun node -> node.color <- c);
  ...
  and loopo donep =
    match
      List.filter
        (fun node ->
          has_either_color BLACK YELLOW node
...
let schedule =
  ...
  and schedule_connected alist =
    match partition alist with a, b -> Seq (schedule_alist a, schedule_alist b)
  in
...
let partition_precomputations alist =
  ...
  let rec loop () =
    match
      List.filter
        (fun node ->
          has_color RED node && List.exists (has_color BLUE) node.successors)
        dag'
    with
    | [] -> ()
    | i ->
