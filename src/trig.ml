...
let dht sign n input =
  ...
  fun i -> Complex.plus [ Complex.real (f i); Complex.imag (f i) ]

let trigI n input =
  let twon = 2 * n in
  let input' = Complex.hermitian twon input in
  Fft.dft 1 twon input'

let interleave_zero input i =
  if i mod 2 == 0 then Complex.zero else input ((i - 1) / 2)

let trigII n input =
  let fourn = 4 * n in
  let input' = Complex.hermitian fourn (interleave_zero input) in
...
let trigIII n input =
  ...
  let twon = 2 * n in
  let input' =
    Complex.hermitian fourn (fun i ->
        if i == 0 then Complex.real (input 0)
        else if i == twon then Complex.uminus (Complex.real (input 0))
        else Complex.antihermitian twon input i)
  in
  let dft = Fft.dft 1 fourn input' in
  fun k -> dft ((2 * k) + 1)

let zero_extend n input i = if i >= 0 && i < n then input i else Complex.zero

let trigIV n input =
  let fourn = 4 * n and eightn = 8 * n in
  let input' =
    Complex.hermitian eightn
      (zero_extend fourn (Complex.antihermitian fourn (interleave_zero input)))
  in
  let dft = Fft.dft 1 eightn input' in
  fun k -> dft ((2 * k) + 1)

let make_dct scale nshift trig n input =
  trig (n - nshift) (Complex.real @@ Complex.times scale @@ zero_extend n input)
...
let shift s input i = input (i - s)

(* DST-x input := TRIG-x (input / i) *)
let make_dst scale nshift kshift jshift trig n input =
  Complex.real
  @@ shift (-jshift)
       (trig (n + nshift)
          ( Complex.uminus @@ Complex.times Complex.i @@ Complex.times scale
          @@ Complex.real
