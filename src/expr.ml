...
let hash_float x =
  ...
  truncate ((float_of_int exponent *. 1234.567) +. (mantissa *. 10000.0))
...
let is_known_constant = function Num _ -> true | NaN _ -> true | _ -> false
...
let rec to_string = function
  ...
  | Store (v, a) -> "(:= " ^ Variable.unparse v ^ " " ^ to_string a ^ ")"
...
let rec to_string_a d x =
  if d = 0 then "..."
    ...
    | NaN n -> string_of_transcendent n
    | Plus x ->
        "(+ " ^ foldr_string_concat (List.map (to_string_a (d - 1)) x) ^ ")"
    | Times (a, b) ->
        "(* " ^ to_string_a (d - 1) a ^ " " ^ to_string_a (d - 1) b ^ ")"
    | CTimes (a, b) ->
        "(c* " ^ to_string_a (d - 1) a ^ " " ^ to_string_a (d - 1) b ^ ")"
    | CTimesJ (a, b) ->
        "(cj* " ^ to_string_a (d - 1) a ^ " " ^ to_string_a (d - 1) b ^ ")"
    | Uminus a -> "(- " ^ to_string_a (d - 1) a ^ ")"
    | Store (v, a) ->
        "(:= " ^ Variable.unparse v ^ " " ^ to_string_a (d - 1) a ^ ")"
...
let add_float_key_value list_so_far k =
  if List.exists (fun k2 -> Number.equal k k2) list_so_far then list_so_far
  else k :: list_so_far
