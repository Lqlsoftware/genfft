...
let rec dft_prime sign n input =
  let sum filter i =
    sigma 0 n (fun j ->
        let coeff = filter (exp n (sign * i * j)) in
        coeff @* input j)
  in
  ...
  and computation_odd =
    ...
    array n (fun i ->
        if i = 0 then
          ...
          input 0 @+ sigma 1 ((n + 1) / 2) (fun j -> input j @+ input (n - j))
        else
          let i' = min i (n - i) in
          if i < n - i then sumr i' @+ sumi i' else sumr i' @- sumi i')
  in
  if n >= !Magic.rader_min then dft_rader sign n input
  else if n == 2 then computation_even
  else computation_odd
...
and dft_rader sign p input =
  ...
  and make_product n a b =
    ...
    array n (fun i -> a i @* scale_factor @* b i)
  in
  ...
  let gen_convolution_by_fft n a b addtoall =
    let fft_a = dft 1 n a and fft_b = dft 1 n b in

    let fft_ab = make_product n fft_a fft_b
    and dc_term i = if i == 0 then addtoall else zero in

    let fft_ab1 = array n (fun i -> fft_ab i @+ dc_term i) and sum = fft_a 0 in
  ...
  and gen_convolution_by_fft_alt n a b addtoall =
    ...
    and bm = array n (fun i -> half (b i @- b ((n - i) mod n))) in
    ...
    let conv1 = dft (-1) n fft_ab1 and conv2 = dft (-1) n fft_ab2 in
    let conv = array n (fun i -> conv1 i @+ conv2 i) in
    (sum, conv)
  ...
  let gen_convolution =
    if p <= !Magic.alternate_convolution then gen_convolution_by_fft_alt
    else gen_convolution_by_fft
  ...
  and output_perm = array p (fun i -> pow_mod ginv i p) in
  let sum, conv = gen_convolution (p - 1) input_perm omega_perm (input 0) in
  array p (fun i ->
      if i = 0 then input 0 @+ sum
      else
        let i' = suchthat 0 (fun i' -> i = output_perm i') in
        conv i')
...
and newsplit sign n input =
  ...
  let sdiv2 n k = s n k @* sinv (2 * n) k (* s(n,k) / s(2*n,k) *)
  and sdiv4 n k =
    (* s(n,k) / s(4*n,k) *)
  ...
  let t n k = exp n k @* sdiv4 (n / 4) k
  and dft1 input = input
  and dft2 input = array 2 (fun k -> input 0 @+ input 1 @* exp 2 k) in

  let rec newsplit0 sign n input =
    if n == 1 then dft1 input
    else if n == 2 then dft2 input
    else
      let u = newsplit0 sign (n / 2) (fun i -> input (i * 2))
  ...
  and newsplitS sign n input =
    if n == 1 then dft1 input
      ...
      let u = newsplitS2 sign (n / 2) (fun i -> input (i * 2))
      and z = newsplitS sign (n / 4) (fun i -> input ((i * 4) + 1))
      and z' =
        newsplitS sign (n / 4) (fun i -> input ((n + (i * 4) - 1) mod n))
      in
  ...
  and newsplitS2 sign n input =
    if n == 1 then dft1 input
      ...
      let u = newsplitS4 sign (n / 2) (fun i -> input (i * 2))
      and z = newsplitS sign (n / 4) (fun i -> input ((i * 4) + 1))
      and z' =
        newsplitS sign (n / 4) (fun i -> input ((n + (i * 4) - 1) mod n))
      in
  ...
  newsplit0 sign n input

and dft sign n input =
  let rec cooley_tukey sign n1 n2 input =
    let tmp1 =
      array n2 (fun i2 -> dft sign n1 (fun i1 -> input ((i1 * n2) + i2)))
    in
    let tmp2 =
      array n1 (fun i1 ->
          array n2 (fun i2 -> exp n (sign * i1 * i2) @* tmp1 i2 i1))
    in
  ...
  and split_radix_dit sign n input =
    ...
    let g10 = array n (fun k -> exp n (sign * k) @* f10 (k mod (n / 4)))
    and g11 = array n (fun k -> exp n (-sign * k) @* f11 (k mod (n / 4))) in
  ...
  and split_radix_dif sign n input =
    ...
    and x11 = array n4 (fun i -> input (i + n4) @- input (i + n2 + n4)) in
    let x1 k i = exp n (k * i * sign) @* x10 i @+ exp 4 (k * sign) @* x11 i in
    let f0 = dft sign n2 x0 and f1 = array 4 (fun k -> dft sign n4 (x1 k)) in
    array n (fun k ->
  ...
  and prime_factor sign n1 n2 input =
    ...
    in
    let tmp2 = array n1 (fun i1 -> dft sign n2 (fun k2 -> tmp1 k2 i1)) in
    fun i -> tmp2 (i mod n1) (i mod n2)
  in

  let algorithm sign n =
    let r = choose_factor n in
    if List.mem n !Magic.rader_list then (* special cases *)
      dft_rader sign n
    else if r == 1 then (* n is prime *)
      dft_prime sign n
    else if gcd r (n / r) == 1 then prime_factor sign r (n / r)
    else if n mod 4 = 0 && n > 4 then
      if !Magic.newsplit && is_power_of_two n then newsplit sign n
      else if !Magic.dif_split_radix then split_radix_dif sign n
      else split_radix_dit sign n
    else cooley_tukey sign r (n / r)
  in
