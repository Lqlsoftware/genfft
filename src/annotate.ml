...
let union l =
  let f x = addelem x (* let is source of polymorphism *) in
  List.fold_right f l
...
let reorder l =
  let rec loop = function
    ...
    | (a, va) :: b ->
        let c =
          List.map (fun (a, x) -> ((a, x), (overlap va x, List.length x))) b
        in
        let c' =
          List.sort
            (fun (_, (a, la)) (_, (b, lb)) ->
...
let rec linearize = function
  ...
  | Par l ->
      ...
      let rec loop n a b =
        if n = 0 then (List.rev b, a)
        else
          match a with
          | [] -> failwith "loop"
          | x :: y -> loop (n - 1) y (x :: b)
      in
      let a, b = loop n2 (reorder l) [] in
      linearize (Seq (Par a, Par b))
  | x -> x

let subset a b = List.for_all (fun x -> List.exists (fun y -> x == y) b) a

let use_same_vars (Assign (av, ax)) (Assign (bv, bx)) =
  is_temporary av && is_temporary bv
  &&
...
let store_to_same_class (Assign (av, ax)) (Assign (bv, bx)) =
  is_locative av && is_locative bv && Variable.same_class av bv

let loads_from_same_class (Assign (av, ax)) (Assign (bv, bx)) =
  match (ax, bx) with
  | Load a, Load b when Variable.is_locative a && Variable.is_locative b ->
...
let rec sched_to_ilist = function
  ...
  | _ -> failwith "sched_to_ilist"

(* Par blocks removed by linearize *)
...
let rec delay_friends friendp sched =
  ...
  in
  match recur (sched_to_ilist sched) sched with
...
let collect_buddy_stores buddy_list sched =
  let rec recur sched delayed_stores =
    match sched with
                       ...
                       ( List.hd buddies,
                         Times
                           ( NaN MULTI_B,
                             Plus
                               (List.map
                                  (fun buddy -> List.assq buddy delayed_stores)
                                  buddies) ) )) ),
              delayed_stores )
...
let schedule_for_pipeline sched =
  ...
  and readyp t ready_times (Assign (_, x)) =
    List.for_all
      (fun var -> try List.assq var ready_times <= t with Not_found -> false)
  ...
  let rec recur sched t ready_times delayed_instructions =
    ...
    | a :: b ->
        let sched, t, ready_times, delayed_instructions =
          recur sched (t + 1) (update_readytimes t a ready_times) (b @ not_ready)
        in
    ...
    | _ -> (
        ...
        | Instr a ->
            if readyp t ready_times a then
              ( sched,
                t + 1,
                update_readytimes t a ready_times,
                delayed_instructions )
            else (Done, t, ready_times, a :: delayed_instructions)
        | Seq (a, b) ->
            ...
            in
            (Seq (a, b), t, ready_times, delayed_instructions)
        | _ -> failwith "schedule_for_pipeline" )
  in
  let rec recur_until_done sched t ready_times delayed_instructions =
    let sched, t, ready_times, delayed_instructions =
      recur sched t ready_times delayed_instructions
    in
    match delayed_instructions with
    | [] -> sched
    | _ ->
        Seq
          (sched, recur_until_done Done (t + 1) ready_times delayed_instructions)
  in
  recur_until_done sched 0 [] []
...
let rec rewrite_declarations force_declarations
  ...
  let declare_it declared =
    if force_declarations || List.length declared >= m then ([], declared)
    else (declared, [])
  in
  ...
  | ASeq (a, b) ->
      ...
      and mb = rewrite_declarations false b in
      let (Annotate (_, ua, _, _, _)) = ma
      and (Annotate (_, ub, _, _, _)) = mb in
      let u, d = declare_it (declared @ ua @ ub) in
      Annotate ([], u, d, 0, ASeq (ma, mb))
...
let annotate list_of_buddy_stores schedule =
  let rec analyze live_at_end = function
    ...
    | Instr i -> (
        ...
        | Assign (v, x) ->
            ...
            Annotate
              (Util.remove v (union live_at_end vars), [ v ], [], 0, AInstr i) )
    | Seq (a, b) ->
  ...
  let x =
    if !Magic.reorder_insns then linearize (anticipate_friends use_same_vars x)
    else x
  in
...
let rec dump print (Annotate (_, _, _, _, code)) = dump_code print code
