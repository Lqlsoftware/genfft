 ...
 * C program structure 
 ***********************************)
type c_decl = Decl of string * string | Tdecl of string

(* arbitrary text declaration *)
and c_ast =
...
let rec unparse_expr_c =
  ...
  let rec unparse_plus maybe =
    ...
    | Uminus c :: Times (a, b) :: d ->
        maybep ^ op "FMS" a b c ^ unparse_plus yes d
    | Times (a, b) :: c :: d -> maybep ^ op "FMA" a b c ^ unparse_plus yes d
    | c :: Times (a, b) :: d -> maybep ^ op "FMA" a b c ^ unparse_plus yes d
    | Uminus a :: b -> " - " ^ parenthesize a ^ unparse_plus yes b
    | a :: b -> maybep ^ parenthesize a ^ unparse_plus yes b
  and parenthesize x =
    match x with
...
and unparse_expr x =
  if !Magic.generic_arith then unparse_expr_generic x else unparse_expr_c x
...
and unparse_annotated force_bracket =
  let rec unparse_code = function
    ...
    | ASeq (a, b) -> unparse_annotated false a ^ unparse_annotated false b
  and declare_variables l =
    ...
    let rec vvar l =
      ...
      if List.length l <= s then
        match l with [] -> "" | _ -> extended_realtype ^ " " ^ uvar l
      else vvar (Util.take s l) ^ vvar (Util.drop s l)
    in
    vvar (List.filter Variable.is_temporary l)
  in
  function
  | Annotate (_, _, decl, _, code) ->
      if (not force_bracket) && Util.null decl then unparse_code code
...
and unparse_ast =
  ...
  | For (a, b, c, d) ->
      "for (" ^ unparse_ast a ^ "; " ^ unparse_ast b ^ "; " ^ unparse_ast c
      ^ ")" ^ unparse_ast d
  | If (a, d) -> "if (" ^ unparse_ast a ^ ")" ^ unparse_ast d
  | Block (d, s) ->
      if s == [] then ""
      else
        "{\n"
 ...
 **************************************************************)
let rec fcn_to_expr_list (Fcn (_, _, _, body)) = ast_to_expr_list body

and acode_to_expr_list = function
  | AInstr (Assign (_, x)) -> [ x ]
  | ASeq (a, b) -> asched_to_expr_list a @ asched_to_expr_list b
  | _ -> []

and asched_to_expr_list (Annotate (_, _, _, _, code)) = acode_to_expr_list code
...
let extract_constants f =
  let constlist = flatten (map expr_to_constants (ast_to_expr_list f)) in
  map
    (fun n ->
      Tdecl ("DK(" ^ Number.to_konst n ^ ", " ^ Number.to_string n ^ ");\n"))
...
let count_stack_vars =
  ...
  and count_ast = function
    ...
    | _ -> 0
  in
  function Fcn (_, _, _, body) -> count_ast body

let count_memory_acc f =
  let rec count_var v = if Variable.is_locative v then 1 else 0
  and count_acode = function
    | AInstr (Assign (v, _)) -> count_var v
    | ASeq (a, b) -> count_asched a + count_asched b
    | _ -> 0
  and count_asched = function Annotate (_, _, _, _, code) -> count_acode code
  and count_ast = function
    | Asch a -> count_asched a
  ...
  and count_acc_expr_func acc = function
    ...
    | _ -> acc
  in
  let (Fcn (typ, name, args, body)) = f in
  count_ast body + fold_left count_acc_expr_func 0 (fcn_to_expr_list f)
...
let rec count_flops_expr_func (adds, mults, fmas) = function
  ...
  | Plus ([ _; _ ] as a) -> (
      ...
      | None ->
          fold_left count_flops_expr_func (adds + length a - 1, mults, fmas) a
      | Some (a, b, c) ->
  ...
  | Times (a, b) ->
      fold_left count_flops_expr_func (adds, mults + 1, fmas) [ a; b ]
  | CTimes (a, b) ->
...
let count_constants f =
  length
    (unique_constants (flatten (map expr_to_constants (fcn_to_expr_list f))))

let arith_complexity f =
  let a, m, fmas = count_flops f
  and v = count_stack_vars f
  and c = count_constants f
  and mem = count_memory_acc f in
  (a, m, fmas, v, c, mem)

(* print the operation costs *)
let print_cost f =
  let (Fcn (_, _, _, _)) = f and a, m, fmas, v, c, mem = arith_complexity f in
  "/*\n" ^ " * This function contains "
  ^ string_of_int (a + fmas)
  ^ " FP additions, "
  ^ string_of_int (m + fmas)
  ^ " FP multiplications,\n" ^ " * (or, " ^ string_of_int a ^ " additions, "
  ^ string_of_int m ^ " multiplications, " ^ string_of_int fmas
  ^ " fused multiply/add),\n" ^ " * " ^ string_of_int v ^ " stack variables, "
  ^ string_of_int c ^ " constants, and " ^ string_of_int mem
  ^ " memory accesses\n" ^ " */\n"
...
let rec simplify_stride stride i =
  ...
  | SNeg x, i -> (
      match simplify_stride x i with Negative y -> y | y -> Negative y )

let rec cstride_to_string = function
  | Simple i -> string_of_int i
  | Constant (s, i) ->
      if !Magic.lisp_syntax then "(* " ^ s ^ " " ^ string_of_int i ^ ")"
      else s ^ " * " ^ string_of_int i
  | Composite (s, i) ->
      if !Magic.lisp_syntax then "(* " ^ s ^ " " ^ string_of_int i ^ ")"
      else "WS(" ^ s ^ ", " ^ string_of_int i ^ ")"
  | Negative x -> "-" ^ cstride_to_string x

let aref name index =
  if !Magic.lisp_syntax then Printf.sprintf "(aref %s %s)" name index
  else Printf.sprintf "%s[%s]" name index
...
let varray_subscript name vstride stride v i =
  ...
  and iindex = simplify_stride stride i in
  let index =
