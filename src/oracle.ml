...
let make_memoizer hash equal =
  let table = ref Assoctable.empty in
  fun f k ->
...
let make_random_oracle () =
  make_memoizer Variable.hash Variable.same (fun _ ->
      float (Random.bits ()) /. 1073741824.0)
...
let likely_equal a b =
  let tolerance = 1.0e-8 and ntests = 20 in
  let rec loop n =
    if n = 0 then true
    else
      let r = make_random_oracle () in
      let va = eval_aux r a and vb = eval_aux r b in
      if
        abs_float (va -. vb)
