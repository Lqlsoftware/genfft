...
module StateMonad = struct
  ...
  let rec mapM f = function
    ...
    | a :: b ->
        f a >>= fun a' ->
        mapM f b >>= fun b' -> returnM (a' :: b')

  let runM m x initial_state =
    let a, _ = m x initial_state in
    a

  let fetchState s = (s, s)

  let storeState newState _ = ((), newState)
end

(* monad with built-in memoizing capabilities *)
module MemoMonad = struct
  open StateMonad
  ...
  let memoizing lookupM insertM f k =
    ...
    | None -> f k >>= fun value -> insertM k value >> returnM value
