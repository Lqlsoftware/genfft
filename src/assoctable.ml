...
let insert hash key value table =
  ...
  let rec ins = function
    ...
    | Node (hash_key, left, right, this_list) ->
        if hash_key < h then Node (hash_key, ins left, right, this_list)
        else if hash_key > h then Node (hash_key, left, ins right, this_list)
        else Node (hash_key, left, right, (key, value) :: this_list)
  in
  ins table
