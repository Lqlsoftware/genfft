...
let inverse_int n =
  CE (makeNum (Number.div Number.one (Number.of_int n)), makeNum Number.zero)

let inverse_int_sqrt n =
  CE
    ( makeNum (Number.div Number.one (Number.sqrt (Number.of_int n))),
      makeNum Number.zero )

let int_sqrt n =
  CE (makeNum (Number.sqrt (Number.of_int n)), makeNum Number.zero)
...
let times3x3 (CE (a, b)) (CE (c, d)) =
  CE
    ( makePlus
        [
          makeTimes (c, makePlus [ a; makeUminus b ]);
...
let times (CE (a, b)) (CE (c, d)) =
  if not !Magic.threemult then
    CE
      ( makePlus [ makeTimes (a, c); makeUminus (makeTimes (b, d)) ],
        makePlus [ makeTimes (a, d); makeTimes (b, c) ] )
  else if is_constant c && is_constant d then times3x3 (CE (a, b)) (CE (c, d))
  else
    (* hope a and b are constant expressions *)
    times3x3 (CE (c, d)) (CE (a, b))

let ctimes (CE (a, _)) (CE (c, _)) = CE (CTimes (a, c), makeNum Number.zero)

let ctimesj (CE (a, _)) (CE (c, _)) = CE (CTimesJ (a, c), makeNum Number.zero)

(* complex exponential (of root of unity); returns exp(2*pi*i/n * m) *)
let exp n i =
  let c, s = Number.cexp n i in
  CE (makeNum c, makeNum s)

(* various trig functions evaluated at (2*pi*i/n * m) *)
let sec n m =
  let c, s = Number.cexp n m in
  CE (makeNum (Number.div Number.one c), makeNum Number.zero)

let csc n m =
  let c, s = Number.cexp n m in
  CE (makeNum (Number.div Number.one s), makeNum Number.zero)

let tan n m =
  let c, s = Number.cexp n m in
  CE (makeNum (Number.div s c), makeNum Number.zero)

let cot n m =
  let c, s = Number.cexp n m in
  CE (makeNum (Number.div c s), makeNum Number.zero)
