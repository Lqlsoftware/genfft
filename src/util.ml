...
let invmod n m =
  let rec loop i = if i * n mod m == 1 then i else loop (i + 1) in
  loop 1
...
let rec gcd n m =
  if n > m then gcd m n
    ...
    let r = m mod n in
    if r == 0 then n else gcd r n

(* reduce the fraction m/n to lowest terms, modulo factors of n/n *)
let lowest_terms n m =
  if m mod n == 0 then (1, 0)
  else
    let nn = abs n in
    let mm = m * (n / nn) in
    let mpos =
      if mm > 0 then mm mod nn else (mm + ((1 + (abs mm / nn)) * nn)) mod nn
    and d = gcd nn (abs mm) in
    (nn / d, mpos / d)
...
let find_generator p =
  let rec period x prod =
    if prod == 1 then 1 else 1 + period x (prod * x mod p)
  in
  let rec findgen x =
...
let rec forall id combiner a b f =
  if a >= b then id else combiner (f a) (forall id combiner (a + 1) b f)
...
let rec find_elem condition = function
  | [] -> None
  | elem :: rest ->
      if condition elem then Some elem else find_elem condition rest

(* find x, x >= a, such that (p x) is true *)
let rec suchthat a pred = if pred a then a else suchthat (a + 1) pred

(* print an information message *)
let info string =
  if !Magic.verbose then (
    let now = Unix.times () and pid = Unix.getpid () in
    prerr_string
      ( string_of_int pid ^ ": " ^ "at t = "
      ^ string_of_float now.tms_utime
      ^ " : " );
    prerr_string (string ^ "\n");
...
let either a b = match a with Some x -> x | _ -> b
