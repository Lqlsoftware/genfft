...
module AlgSimp : sig
  ...
  let fetchSimp = fetchState >>= fun (s, _) -> returnM s

  let storeSimp s = fetchState >>= fun (_, c) -> storeState (s, c)

  let lookupSimpM key =
    fetchSimp >>= fun table -> returnM (node_lookup key table)

  let insertSimpM key value =
    fetchSimp >>= fun table -> storeSimp (node_insert key value table)

  let subset a b = List.for_all (fun x -> List.exists (fun y -> x == y) b) a
  ...
  let hashCSE x = if !Magic.randomized_cse then Oracle.hash x else Expr.hash x

  let equalCSE a b =
    if !Magic.randomized_cse then
      structurallyEqualCSE a b || Oracle.likely_equal a b
    else structurallyEqualCSE a b

  let fetchCSE = fetchState >>= fun (_, c) -> returnM c

  let storeCSE c = fetchState >>= fun (s, _) -> storeState (s, c)
  ...
  let rec snumM = function
    | n when Number.is_zero n -> makeNode (Num Number.zero)
    | n when Number.negative n -> makeNode (Num (Number.negate n)) >>= suminusM
    | n -> makeNode (Num n)
  ...
  and stimesM = function
    ...
    | NaN I, CTimes (a, b) -> stimesM (NaN I, b) >>= fun ib -> sctimesM (a, ib)
    | NaN I, CTimesJ (a, b) -> stimesM (NaN I, b) >>= fun ib -> sctimesjM (a, ib)
  ...
  and reduce_sumM x =
    match x with
    | [] -> returnM []
    | [ Num a ] -> if Number.is_zero a then returnM [] else returnM x
    | [ Uminus (Num a) ] -> if Number.is_zero a then returnM [] else returnM x
    | Num a :: Num b :: s ->
        snumM (Number.add a b) >>= fun x -> reduce_sumM (x :: s)
    | Num a :: Uminus (Num b) :: s ->
        snumM (Number.sub a b) >>= fun x -> reduce_sumM (x :: s)
    | Uminus (Num a) :: Num b :: s ->
        snumM (Number.sub b a) >>= fun x -> reduce_sumM (x :: s)
    | Uminus (Num a) :: Uminus (Num b) :: s ->
        snumM (Number.add a b) >>= suminusM >>= fun x -> reduce_sumM (x :: s)
    | (Num _ as a) :: b :: s -> reduce_sumM (b :: a :: s)
    | (Uminus (Num _) as a) :: b :: s -> reduce_sumM (b :: a :: s)
    | a :: s -> reduce_sumM s >>= fun s' -> returnM (a :: s')
  ...
  and collectM which x =
    let rec findCoeffM which = function
      ...
      | Uminus x ->
          ...
          suminusM coeff >>= fun mcoeff -> returnM (mcoeff, b)
    ...
    | a :: b ->
        ...
        splusM w >>= fun w' ->
        stimesM (w', xpr) >>= fun t' -> returnM (t' :: wo')

  and mangleSumM x =
    returnM x >>= reduce_sumM
  ...
  and reorder_uminus = function
    (* push all Uminuses to the end *)
    | [] -> []
  ...
  and generateFusedMultAddM =
    ...
    fun l ->
      if !Magic.enable_fma && count is_multiplication l >= 2 then
        ...
        stimesM (max', pw') >>= fun mw' -> splusM (wo @ [ mw' ])
      else makeNode (Plus l)

  and negative = function Uminus _ -> true | _ -> false
  ...
  and deepCollectM maxdepth l =
    ...
    and duplicates = function
      | [] -> []
      | a :: b -> if List.memq a b then a :: duplicates b else duplicates b
    in

    let rec splitDuplicates depth d x =
      if List.memq x d then snumM Number.zero >>= fun zero -> returnM (zero, x)
      else
        match x with
        | Times (a, b) ->
            ...
            splusM [ xa'; xb'; xab ] >>= fun x -> returnM (ab, x)
        | Uminus a ->
            splitDuplicates depth d a >>= fun (x, y) ->
            suminusM x >>= fun ux ->
            suminusM y >>= fun uy -> returnM (ux, uy)
        | Plus l when depth > 0 ->
            mapM (splitDuplicates (depth - 1) d) l >>= fun ld ->
            let l', d' = List.split ld in
            splusM l' >>= fun p ->
            splusM d' >>= fun d'' -> returnM (p, d'')
        | x -> snumM Number.zero >>= fun zero' -> returnM (x, zero')
    in

    let l' = List.flatten (List.map (findTerms maxdepth) l) in
    match duplicates l' with
    | [] -> returnM l
    | d ->
        ...
        let rec flattenPlusM = function
          | Plus l -> returnM l
          | Uminus x -> flattenPlusM x >>= mapM suminusM
          | x -> returnM [ x ]
        in
        mapM flattenPlusM d' >>= fun d'' ->
        splusM (List.flatten d'') >>= fun d''' -> mangleSumM [ l''; d''' ]
  ...
  and splusM l =
    ...
    if not (List.exists negative l') then canonicalizeM l'
      ...
      match fma_heuristics l' with
      | Some true -> mapM suminusM l' >>= splusM >>= suminusM
      | Some false -> canonicalizeM l'
      | None ->
          (* Ask the Oracle for the canonical form *)
          if (not !Magic.randomized_cse) && Oracle.should_flip_sign (Plus l')
          then mapM suminusM l' >>= splusM >>= suminusM
          else canonicalizeM l'
  ...
  let rec algsimpM x =
    memoizing lookupSimpM insertSimpM
      (function
        ...
        | Plus a -> mapM algsimpM a >>= splusM
        | Times (a, b) ->
            algsimpM a >>= fun a' ->
            algsimpM b >>= fun b' -> stimesM (a', b')
        | CTimes (a, b) ->
            algsimpM a >>= fun a' ->
            algsimpM b >>= fun b' -> sctimesM (a', b')
        | CTimesJ (a, b) ->
            algsimpM a >>= fun a' ->
            algsimpM b >>= fun b' -> sctimesjM (a', b')
        | Uminus a -> algsimpM a >>= suminusM
        | Store (v, a) -> algsimpM a >>= fun a' -> makeNode (Store (v, a'))
...
module Transpose = struct
  ...
  let lookupDualsM key =
    fetchDuals >>= fun table -> returnM (node_lookup key table)

  let insertDualsM key value =
    fetchDuals >>= fun table -> storeDuals (node_insert key value table)
  ...
  let make_transposer parent_table =
    let rec termM node candidate_parent =
      ...
      | Times (a, b) when b == node ->
          dualM candidate_parent >>= fun x' -> returnM [ makeTimes (a, x') ]
      | CTimes (a, b) when b == node ->
          dualM candidate_parent >>= fun x' -> returnM [ CTimes (a, x') ]
      | CTimesJ (a, b) when b == node ->
          dualM candidate_parent >>= fun x' -> returnM [ CTimesJ (a, x') ]
      | Uminus n when n == node ->
          dualM candidate_parent >>= fun x' -> returnM [ makeUminus x' ]
      | _ -> returnM []
    and dualExpressionM this_node =
      ...
      >>= fun l -> returnM (makePlus (List.flatten l))
    and dualM this_node =
      memoizing lookupDualsM insertDualsM
        (function
          | Load v as x ->
              if Variable.is_constant v then returnM (Load v)
              else dualExpressionM x >>= fun d -> returnM (Store (v, d))
  ...
  let transpose dag =
    ...
    let all_nodes, parent_table =
      visit [] Assoctable.empty Assoctable.empty dag
    in
...
module Stats : sig
    ...
    let rec loop (load, store, plus, times, uminus, num) = function
      ...
      | node :: rest ->
          loop
            ...
            | Plus x ->
                (load, store, plus + (List.length x - 1), times, uminus, num)
  ...
  let weight (l, s, p, t, u, n) = l + s + (10 * p) + (20 * t) + u + n
...
let algsimp v =
  let rec simplification_loop v =
    ...
    let v =
      ( AlgSimp.algsimp @@ Transpose.transpose @@ AlgSimp.algsimp
      @@ Transpose.transpose )
        v
    in
    let complexity' = Stats.complexity v in
