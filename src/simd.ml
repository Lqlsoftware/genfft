...
and unparse_expr =
  ...
  and op3 nam a b c =
    nam ^ "(" ^ unparse_expr a ^ ", " ^ unparse_expr b ^ ", " ^ unparse_plus c
    ^ ")"
  and op2 nam a b = nam ^ "(" ^ unparse_plus a ^ ", " ^ unparse_plus b ^ ")"
  and op1 nam a = nam ^ "(" ^ unparse_expr a ^ ")"
  and negate = function
    | [] -> []
  ...
  function
  | CTimes (Load tw, src) when Variable.is_constant tw && !Magic.generate_bytw
    ->
      unparse_by_twiddle "BYTW" tw src
  | CTimesJ (Load tw, src) when Variable.is_constant tw && !Magic.generate_bytw
    ->
  ...
  | CTimes (a, b) -> sprintf "VZMUL(%s, %s)" (unparse_expr a) (unparse_expr b)
  | CTimesJ (a, Times (NaN I, b)) ->
      sprintf "VZMULIJ(%s, %s)" (unparse_expr a) (unparse_expr b)
  | CTimesJ (a, b) -> sprintf "VZMULJ(%s, %s)" (unparse_expr a) (unparse_expr b)
...
and unparse_ast ast =
  let rec unparse_assignment = function
    | Assign (v, x) when Variable.is_locative v -> unparse_store v x
    | Assign (v, x) -> Variable.unparse v ^ " = " ^ unparse_expr x ^ ";\n"
    ...
    and declare_variables l =
      ...
      let rec vvar l =
        ...
        if List.length l <= s then
          match l with [] -> "" | _ -> realtype ^ " " ^ uvar l
...
let extract_constants f =
  let constlist = flatten (map expr_to_constants (C.ast_to_expr_list f)) in
  map
    (fun n ->
      Tdecl ("DVK(" ^ Number.to_konst n ^ ", " ^ Number.to_string n ^ ");\n"))
