...
let unique_array_c n = array n (fun _ -> (Unique.make (), Unique.make ()))

let unique_v_array_c veclen n = array veclen (fun _ -> unique_array_c n)
...
let temporary_array n = array n (fun i -> Variable.make_temporary ())

let temporary_array_c n =
  let tmpr = temporary_array n and tmpi = temporary_array n in
  array n (fun i -> (tmpr i, tmpi i))

let temporary_v_array_c veclen n = array veclen (fun v -> temporary_array_c n)
...
let twiddle_array nt w =
  array (nt / 2) (fun i ->
      ...
      let refr, refi =
        ( C.array_subscript w stride (2 * i),
          C.array_subscript w stride ((2 * i) + 1) )
      in
      let kr, ki =
        (Variable.make_constant klass refr, Variable.make_constant klass refi)
      in
      load_c (kr, ki))
...
let load_array_hc n var =
  array n (fun i ->
      if i < n - i then load_c (var i)
      else if i > n - i then Complex.times Complex.i (load_c (var (n - i)))
      else load_r (var i))

let load_v_array_c veclen n var = array veclen (fun v -> load_array_c n (var v))
...
let assign_array_c n dst src =
  List.flatten
    (rmap (iota n) (fun i ->
         let ar, ai = Complex.assign (dst i) (src i) in
         [ ar; ai ]))

let assign_v_array_c veclen n dst src =
  List.flatten (rmap (iota veclen) (fun v -> assign_array_c n (dst v) (src v)))

let vassign_v_array_c veclen n dst src =
  List.flatten
    (rmap (iota n) (fun i ->
         List.flatten
           (rmap (iota veclen) (fun v ->
                let ar, ai = Complex.assign (dst v i) (src v i) in
                [ ar; ai ]))))

let store_array_r n dst src = rmap (iota n) (fun i -> store_r (dst i) (src i))

let store_array_c n dst src =
  List.flatten (rmap (iota n) (fun i -> store_c (dst i) (src i)))

let store_array_hc n dst src =
  List.flatten
    (rmap (iota n) (fun i ->
         if i < n - i then store_c (dst i) (src i)
         else if i > n - i then []
         else [ store_r (dst i) (Complex.real (src i)) ]))

let store_v_array_c veclen n dst src =
  List.flatten (rmap (iota veclen) (fun v -> store_array_c n (dst v) (src v)))
...
let elementwise_v f veclen n a =
  array veclen (fun v -> array n (fun i -> f (a v i)))
...
let speclist =
  [
    ...
    ( "-sign",
      Arg.Int (fun i -> if i > 0 then sign := 1 else sign := -1),
      " sign of transform" );
  ]
...
let declare_register_fcn name =
  if name = "" then "void NAME(planner *p)\n"
    ...
    "void " ^ choose_simd "X" "XSIMD" ^ "(codelet_" ^ name ^ ")(planner *p)\n"

let declare_twinstr name = "X(TW_" ^ String.uppercase_ascii name ^ ")"

let stringify name =
  if name = "" then "STRINGIZE(NAME)"
  else choose_simd ("\"" ^ name ^ "\"") ("XSIMD_STRING(\"" ^ name ^ "\")")

let parse user_speclist usage =
  Arg.parse
    (user_speclist @ speclist @ Magic.speclist @ Simdmagic.speclist)
    standard_arg_parse_fail usage
...
let stride_fixed = function Stride_variable -> false | _ -> true

let arg_to_stride s =
  try Fixed_int (int_of_string s)
  with Failure "int_of_string" -> Fixed_string s
...
let unparse tree =
  ...
  ^ ( if String.length !Magic.inklude > 0 then
      Printf.sprintf "#include \"%s\"\n\n" !Magic.inklude
    else "" )
  ^
  if !Simdmagic.simd_mode then Simd.unparse_function tree
  else C.unparse_function tree
...
let finalize_fcn ast =
  ...
  and extract_constants =
    if !Simdmagic.simd_mode then Simd.extract_constants else C.extract_constants
  in

  mergedecls (C.Block (extract_constants ast, [ ast; C.Simd_leavefun ]))

let twinstr_to_string vl x =
  if !Simdmagic.simd_mode then Twiddle.twinstr_to_simd_string vl x
  else Twiddle.twinstr_to_c_string x
